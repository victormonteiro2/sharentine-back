...
router.get("/profile-edit", passport.authenticate("jwt", { session: false }), (req, res, next) => {
  res.json({
    user: req.user,
    token: req.query.secret_token,
  });
...
router.delete("/profile/:id", async (req, res) => {
  try {
    ...
    const commentResult = await Comment.deleteMany({ user: id });

    console.log("Result delete =>", commentResult);
